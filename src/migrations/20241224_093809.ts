...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`products_additional_information_sections_section_content\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`attribute_name\` text,
  	\`attribute_value\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`products_additional_information_sections\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`products_additional_information_sections_section_content_order_idx\` ON \`products_additional_information_sections_section_content\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`products_additional_information_sections_section_content_parent_id_idx\` ON \`products_additional_information_sections_section_content\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`products_additional_information_sections\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`section_title\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`products_additional_information_sections_order_idx\` ON \`products_additional_information_sections\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`products_additional_information_sections_parent_id_idx\` ON \`products_additional_information_sections\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_products_v_version_additional_information_sections_section_content\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`attribute_name\` text,
  	\`attribute_value\` text,
  	\`_uuid\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`_products_v_version_additional_information_sections\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`_products_v_version_additional_information_sections_section_content_order_idx\` ON \`_products_v_version_additional_information_sections_section_content\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_products_v_version_additional_information_sections_section_content_parent_id_idx\` ON \`_products_v_version_additional_information_sections_section_content\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_products_v_version_additional_information_sections\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`section_title\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_products_v_version_additional_information_sections_order_idx\` ON \`_products_v_version_additional_information_sections\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_products_v_version_additional_information_sections_parent_id_idx\` ON \`_products_v_version_additional_information_sections\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_orders_v\` (
  	...
  	\`version_placed_at\` text,
  	\`version_payment_method\` text,
  	\`version_shipping_address_address_line1\` text,
  	\`version_shipping_address_address_line2\` text,
  	\`version_shipping_address_city\` text,
  	\`version_shipping_address_state\` text,
  	\`version_shipping_address_zip_code\` text,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`products_additional_information_sections_section_content\`;`)
  await db.run(sql`DROP TABLE \`products_additional_information_sections\`;`)
  ...
  await db.run(sql`DROP TABLE \`_products_v_version_additional_information_sections_section_content\`;`)
  await db.run(sql`DROP TABLE \`_products_v_version_additional_information_sections\`;`)
